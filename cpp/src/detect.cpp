...
bool detect(const char *input_path,const char *output_path,int inner_window_width,int outer_window_width,double threshold) {
	...
	for (long tt=0; tt<N; tt+=chunk_size) {
		...
		if (tt2>N) tt2=N;
		total_num_events+=detect_2(H,input_file,H_out,output_file,tt,tt2,overlap,inner_window_width,outer_window_width,threshold);
	}
