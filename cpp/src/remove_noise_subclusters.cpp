...
QList<long> randsample_with_replacement(long N,long K) {
	QList<long> ret;
	for (int i=0; i<K; i++) {
		ret << qrand()%N;
	}
...
// ch is the channel where detection takes place
	...
	QList<long> rand_times=randsample_with_replacement(N-2*T,num_rand_times);
	...
	for (int i=0; i<rand_times.count(); i++) {
		peaks << rand_clips.value(ch,Tmid,i);
	}
	...
	for (int t=0; t<T; t++) {
		for (int m=0; m<M; m++) {
			double val=noise_shape.value(m,t);
	...
	if (noise_shape_norm) {
		for (int t=0; t<T; t++) {
			for (int m=0; m<M; m++) {
				noise_shape.setValue(noise_shape.value(m,t)/noise_shape_norm,m,t);
			}
		}
	}
	return noise_shape;
...
QList<Subcluster> compute_subcluster_detectability_scores(DiskReadMda &pre,Mda &clips,int channel,const Remove_noise_subclusters_opts &opts) {
	...
	for (int s=0; s<shells.count(); s++) {
		...
		for (int t=0; t<T; t++) {
			for (int m=0; m<M; m++) {
				subtemplate_resid.setValue(subtemplate.value(m,t)-ip*noise_shape.value(m,t),m,t);
			}
		}
		...
		SC.peak=subtemplate.value(channel,Tmid);
		subclusters << SC;
	}
	return subclusters;
}
...
bool remove_noise_subclusters(const char *pre_path,const char *firings_path,const char *firings_out_path,const Remove_noise_subclusters_opts &opts) {
	...
	DiskReadMda pre; pre.setPath(pre_path);
	...
	QList<long> times;
	for (int i=0; i<L; i++) {
		channels << (int)firings.value(0,i)-1; //convert to zero-based indexing
		times << (long)firings.value(1,i) -1; //convert to zero-based indexing
		labels << (int)firings.value(2,i);
	}

	Mda clips=extract_clips(pre,times,T);
	...
	for (int k=1; k<=K; k++) {
		...
		if (inds_k.count()>0) {
			...
			QList<Subcluster> subclusters0=compute_subcluster_detectability_scores(pre,clips_k,channel,opts);
	...
	for (int i=0; i<L; i++) {
		if (events_to_use[i]) {
			for (int jj=0; jj<firings.N1(); jj++) {
				firings_out.setValue(firings.value(jj,i),jj,ee);
			}
			int label=(int)firings_out.value(2,ee);
			if (label) label=labels_map[label];
			firings_out.setValue(label,2,ee);
			ee++;
		}
	}
