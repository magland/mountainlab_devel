function mscmd_create_clips_file(raw_path,firings_path,output_clips_path,output_index_path,opts)
...
%
% Syntax:  mscmd_create_clips_file(input_path,firings_path,output_clips_path,output_index_path,opts)
%
% Inputs:
%    raw_path - path to MxN array of raw or preprocessed data
%    firings_path - path to file containing times/labels information
...
cmd=sprintf('%s create_clips_file --input=%s --firings=%s --output=%s --index_out=%s --clip_size=%d ',mscmd_exe,raw_path,firings_path,output_clips_path,output_index_path,opts.clip_size);
