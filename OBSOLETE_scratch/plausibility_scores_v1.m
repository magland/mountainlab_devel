function firings2=plausibility_scores_v1(X,firings,opts)

if (nargin<1) test_plausibility_scores_v1; return; end;
...
labels=firings(3,:);
peaks=firings(4,:);
scores=zeros(1,L); %plausibility scores
K=max(labels);
...
for k=1:K
    ...
    for s=1:length(shells_k)
        ...
        scores_ks=compute_plausibility_scores(clips_ks,random_clips,opts);
...
function scores=compute_plausibility_scores(clips,random_clips,opts)
[M,T,L]=size(clips);
