...
Mda confusion_matrix_2(const char *firings1_path, const char *firings2_path, int max_matching_offset,QMap<int,int> &map12) {
	...
	for (int ii=0; ii<L1.count(); ii++) {
		if (L1[ii]>K1) K1=L1[ii];
	}
	int K2=1;
	for (int ii=0; ii<L2.count(); ii++) {
		if (L2[ii]>K2) K2=L2[ii];
	}

	int CM[(K1+1)*(K2+1)];
	for (int ii=0; ii<(K1+1)*(K2+1); ii++) CM[ii]=0;

	int pass1=1; int pass2=2;
	if (map12.isEmpty()) pass1=2; //if the map is empty, only do pass 2
	...
	for (int pass=pass1; pass<=pass2; pass++) {
		...
		for (int offset=0; offset<=max_matching_offset; offset++) {
			QSet<int> inds1_to_remove; inds1_to_remove.clear(); //clear almost definitely is not needed here, but I was a bit nervous, better safe than sorry
			QSet<int> inds2_to_remove; inds2_to_remove.clear();
	...
	Mda output; output.allocate(K1+1,K2+1);
